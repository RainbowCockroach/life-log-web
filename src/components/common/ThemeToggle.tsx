  ...
  if (variant === 'toggle') {
    return (
      <button
        ...
        aria-label={`Current theme: ${getThemeLabel(theme)}${isAuto ? ` (${resolvedTheme})` : ''}. Click to change.`}
        title={`Current: ${getThemeLabel(theme)}${isAuto ? ` (currently ${resolvedTheme})` : ''}`}
      >
        <span style={{ fontSize: '16px' }}>{getThemeIcon(theme)}</span>
        {showLabels && (
          <span>
            {getThemeLabel(theme)}
            {isAuto && <span style={{ color: 'var(--secondary-color)', fontSize: '12px', marginLeft: '4px' }}>
              ({resolvedTheme})
            </span>}
          </span>
        )}
    ...
    >
      {(['light', 'dark', 'auto'] as Theme[]).map((themeOption) => (
        <button
          ...
          style={{
            backgroundColor: theme === themeOption ? 'var(--button-hover-background)' : 'var(--button-background)',
            color: 'var(--text-color)',
            border: '1px solid var(--border-color)',
            borderRadius: '4px',
            padding: '4px 8px',
